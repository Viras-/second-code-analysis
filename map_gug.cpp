...
// GUG maps consist of two files:
// * 13.gug: Geospatial and other metadata.
// * 13.gup: Image data, a tile-based variation of the BMP file format.
// GUP files can exist on their own without the GUG, e.g. for logos or legends.
// GUP files can contain multiple images, which is used to store multiple
// resolutions of the same map, to decrease loading times at low zoom levels.
//
// GUG format:
// GUG files are protected by a simple substitution cypher. After decryption,
...
// * The [Header] section (represented by class GUGHeader) contains data like
//   map title and description.
// * Each [MAP*] section (represented by GUGMapInfo) describes one zoom level
//   of the map.
//   Most important parameters:
//   - Path: "Path=<gupname>|<index>", e.g. "Path=13.gup|0"
//     The name of the GUP file along with the index of this resolution within
//     the GUP (multiple resolution images are stored in one GUP).
...
//
// GUP Format:
// One GUP file can contain multiple images (represented by class GUPImage).
...
//     - bfType: 0x5847 ("GX")
//     - bfOffBits: Allows calculating the start of the GUPTileOffset table.
// * BITMAPINFOHEADER (40 bytes): Follows the BFH, ``START + 0x0E``
//     - biSize: Allows calculating the start of the GUPHeader.
//     - biHeight: Might be negative to indicate top-down images.
// * GUPHeader (24 bytes): Follows the BIH, ``START + 0x0E + biSize``
...
//   tile boundary!
// * GUPTileOffset table: ``START + bfOffBits``
//   An array of GUPTileOffset objects. The number of entries is the total
...
//   at the border of images. It's probably best filled with GUGHeader.BkColor.
//
// * The tile buffers: ``START + bfOffBits + toff.offset``
//   Encrypted with the same substitution cypher as GUG files. The offset into
...
//
// * The end of the current GUP Image (and start of the next Image) can be
//   found as:
//
...
static_assert(sizeof(GUPBitmapFileHdr) == 14,
              "GUPBitmapFileHdr has the wrong size.");
static_assert(sizeof(GUPBitmapInfoHdr) == 40,
              "GUPBitmapInfoHdr has the wrong size.");
static_assert(sizeof(GUPHeader) == 24,
              "GUPHeader has the wrong size.");
static_assert(sizeof(GUPTileOffset) == 16,
              "GUPTileOffset has the wrong size.");


// A lookup table to decrypt the GUG files.
...
unsigned int GUGHeader::count_gauges() {
    if (!Gauges.size())
        return 0;
...
bool GUGHeader::set_field(const std::string &key, const std::string &value) {
...
bool GUGMapInfo::set_field(const std::string &key,
...
void GUGMapInfo::Pixel_to_PCS(double x_px, double y_px,
...
void GUGMapInfo::PCS_to_Pixel(double x_pcs, double y_pcs,
...
void GUGMapInfo::complete_initialization() {
...
GUGFile::GUGFile(std::string fname)
: m_fname(fname), m_header(), m_mapinfos(), m_decoded_data()
{
    ...
    if (!data.size())
        throw std::runtime_error("Failed to read GUG metadata.");
    decrypt_buf(data, 0);
    m_decoded_data = data;
    ParseINI(m_decoded_data);
}

void GUGFile::ParseINI(const std::string &data) {
    std::regex re_header("\\[Header\\]");
    std::regex re_map("\\[MAP\\d*\\]");
    std::regex re_keyvalue("(.*)=(.*)");
    ...
    while(std::getline(buf, line)) {
        ...
        if (std::regex_match(line, re_header)) {
            if (pm != PM_NONE)
                ...
                throw std::runtime_error("Multiple [Header] entries in GUG.");
            pm = PM_HEADER;
        } else if (std::regex_match(line, re_map)) {
            if (pm == PM_NONE)
                throw std::runtime_error("No [Header] entry in GUG file.");
            pm = PM_MAPINFO;
            m_mapinfos.push_back(GUGMapInfo());
            ...
            throw std::runtime_error("Could not parse GUG metadata line.");
        }
    }
...
unsigned int GUGFile::BestResolutionIndex() const {
...
std::string GUGFile::ResolvePath(const std::string &gup_name) const {
    size_t last_slash = m_fname.rfind(PATH_SEP);
    if (last_slash == std::string::npos) {
        // No directory separator in GUG path: It is in the current directory.
        // Just return the GUP filename.
        return gup_name;
    }
    return m_fname.substr(0, last_slash + 1) + gup_name;
}

const GUGMapInfo &GUGFile::MapInfo(unsigned int n) const {
    return m_mapinfos[n];
}

std::tuple<std::string, unsigned int> GUGFile::GupPath(unsigned int n) const {
    std::string fname = m_mapinfos[n].Path;
    size_t delim = fname.rfind('|');
    if (delim == std::string::npos) {
        return std::make_tuple(std::string(), 0);
    }
    std::string image_num_str = fname.substr(delim + 1);
    ...
    catch(...) { return std::make_tuple(std::string(), 0); }

    return std::make_tuple(ResolvePath(fname), image_num);
}


GUPImage::GUPImage(const std::string &fname, int index, int64_t foffset) :
    m_file(fname, std::ios::in | std::ios::binary),
    m_fname(fname), m_findex(index), m_foffset(foffset),
    m_bfh(), m_bih_buf(), m_bih(nullptr), m_guphdr(nullptr),
...
GUPImage::GUPImage(const GUPImage &other) :
    ...
    m_bfh(other.m_bfh),
    // Copy over m_bih_buf, make m_bih and m_guphdr point to our own copy.
    // No sanity checks necessary, as we throw in Init() if anything is fishy.
    m_bih_buf(other.m_bih_buf),
    m_bih(reinterpret_cast<const GUPBitmapInfoHdr*>(&m_bih_buf[0])),
    m_guphdr(reinterpret_cast<const GUPHeader*>(&m_bih_buf[m_bih->biSize])),
...
void GUPImage::Init() {
    ...
    m_file.read((char*)&m_bfh, sizeof(GUPBitmapFileHdr));
    if (m_bfh.bfType != 0x5847) {  // "GX"
        throw std::runtime_error("Wrong signature: not a valid GUP image");
    }

    m_file.seekg(m_foffset + sizeof(GUPBitmapFileHdr));
    m_bih_buf.resize(m_bfh.bfOffBits - sizeof(GUPBitmapFileHdr));
    m_file.read(&m_bih_buf[0], m_bih_buf.size());

    if (m_bih_buf.size() < sizeof(GUPBitmapInfoHdr)) {
        throw std::runtime_error("Invalid GUP Bitmap header: BIH missing.");
    }
    m_bih = reinterpret_cast<const GUPBitmapInfoHdr*>(&m_bih_buf[0]);

    if (m_bih_buf.size() < m_bih->biSize + sizeof(GUPHeader)) {
        throw std::runtime_error("Invalid GUP Bitmap header: GUPH missing.");
    }
    m_guphdr = reinterpret_cast<const GUPHeader*>(&m_bih_buf[m_bih->biSize]);

    // The remainder of the header is just padding (\0 bytes).
    // header_rest = m_bih_buf.get() + m_bih->biSize + sizeof(GUPHeader);
    // header_rest_len = m_bfh.bfOffBits - sizeof(GUPBitmapFileHdr) -
    //                   m_bih->biSize - sizeof(GUPHeader);
    ...
    m_file.read((char*)&m_tile_index[0],
                sizeof(struct GUPTileOffset) * m_tiles);
}

int64_t GUPImage::NextImageOffset() const {
...
int GUPImage::AnnouncedWidth() const { return m_bih->biWidth; };
int GUPImage::AnnouncedHeight() const { return abs(m_bih->biHeight); };
int GUPImage::RealWidth() const { return TileWidth() * NumTilesX(); };
int GUPImage::RealHeight() const { return TileHeight() * NumTilesY(); };
int GUPImage::TileWidth() const { return m_guphdr->tile_px_x; };
int GUPImage::TileHeight() const { return m_guphdr->tile_px_y; };
int GUPImage::NumTilesX() const { return m_tiles_x; };
int GUPImage::NumTilesY() const { return m_tiles_y; };

std::string GUPImage::LoadCompressedTile(long tx, long ty) const {
...
PixelBuf GUPImage::LoadTile(long tx, long ty) const {
...
GUPImage MakeGupImage(const std::string& path, unsigned int gup_image_idx) {
    int64_t foffset = 0;
    auto fsize = GetFilesize(path);
    int64_t filesize = GetFilesize(path);
    for (int i = 0; foffset < filesize; i++) {
        GUPImage image(path, i, foffset);
        if (i == gup_image_idx) {
            return image;
        }
        foffset = image.NextImageOffset();
    }
    throw std::runtime_error("Could not find GUP image.");
}

GUPImage MakeBestResolutionGupImage(const GUGFile &gugfile) {
    unsigned int idx_best_res = gugfile.BestResolutionIndex();

    std::string path;
    int image_num;
    std::tie(path, image_num) = gugfile.GupPath(idx_best_res);

    return MakeGupImage(path, image_num);
}


GUGMap::GUGMap(const std::string &fname)
    : m_getregion_mutex(), m_gugfile(fname),
      m_image(MakeBestResolutionGupImage(m_gugfile)),
      m_gugheader(&m_gugfile.Header()),
      m_gugmapinfo(&m_gugfile.MapInfo(m_gugfile.BestResolutionIndex())),
...
unsigned int GUGMap::GetWidth() const {
    return m_width;
}
unsigned int GUGMap::GetHeight() const {
    return m_height;
}
MapPixelDeltaInt GUGMap::GetSize() const {
    return MapPixelDeltaInt(m_width, m_height);
}

PixelBuf
GUGMap::GetRegion(const MapPixelCoordInt &pos,
                  const MapPixelDeltaInt &size) const
{
    PixelBuf oob_background = bg_tile_if_out_of_bounds(*this, pos, size);
    if (oob_background.GetData())
        return oob_background;
...
const std::string &GUGMap::GetFname() const {
    return m_gugfile.Filename();
}

const std::string &GUGMap::GetTitle() const {
    return m_gugfile.Header().Title;
}
